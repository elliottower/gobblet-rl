# from https://github.com/ray-project/ray/blob/master/rllib/examples/rock_paper_scissors_multiagent.py
"""A simple multi-agent env with two agents playing gobblet.
This demonstrates running the following policies in competition:
    (1) LSTM/feedforward PG policies
    (2) LSTM policy with custom entropy loss
"""
...
def env_creator(args):
    env = gobblet_v0.env(render_mode="human")
    return env


register_env("gobblet", lambda config: PettingZooEnv(env_creator(config)))


def run_same_policy(args, stop):
    """Use the same policy for both agents (trivial case)."""
    config = PGConfig().environment("gobblet").framework(args.framework)
